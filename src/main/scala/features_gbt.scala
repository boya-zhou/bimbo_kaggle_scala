...
//    val origin_test = spark.read.option("header","true").option("inferSchema","true").csv(path + "/data/interm/test_sample.csv")
    val path_s3 = "s3a://boya.kaggle.bimbo/train.csv"
    val origin_train = spark.read.option("header","true").option("inferSchema","true").csv(path_s3)
    ...
    def merge_info(origin_train : DataFrame, type_df : String) : DataFrame = {
      ...
      val train_joined = pivot_sc_sp_scp_cp_lag1_lag2.drop("Cliente_ID", "Producto_ID").na.fill(-999)
    ...
    def show_data(df_to_show: DataFrame) : Unit = {
